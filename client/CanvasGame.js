...
export class CanvasGame {
  ...
  onTick() {
    ...
    for (let i = 0; i < this.bullets.length; i++) {
      ...
      for (const player of Object.keys(this.remotePlayers).map((key) => this.remotePlayers[key])) {
        if (Math.abs(bullet.getBounds().x - player.getBounds().x)*2 <= (player.getBounds().width + bullet.getBounds().width)
        && Math.abs(bullet.getBounds().y - player.getBounds().y)*2 <= (player.getBounds().height + bullet.getBounds().height)) {
